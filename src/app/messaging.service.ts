...
export class MessagingService {
  ...
  async init() {
    ...
    const docSnapshot = this.afs.collection<any>('subscriptions', ref => ref.where('endpoint', '==', sub.endpoint)).valueChanges()
      .subscribe(data => {
        if (data.length === 0) {
    ...
    // const docSnapshot = this.subCollectionRef.doc(sub.toJSON()).valueChanges().subscribe((data) => {
    //   if (!data) {
    //     const subscribe = this.addSub(sub.toJSON());
    //     console.log('subscription added!', subscribe);
    //   }
    //   console.log('da!', subscribe);
    // });
